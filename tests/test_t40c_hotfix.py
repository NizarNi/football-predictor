...
def test_unsupported_league_marks_unavailable(monkeypatch):
    from football_predictor import xg_data_fetcher
    ...
    monkeypatch.setattr(
        xg_data_fetcher,
        "_pick_effective_league",
        lambda league, home, away: ("unsupported", None),
    )
    monkeypatch.setattr(
        xg_data_fetcher,
        "fetch_league_xg_stats",
    ...
    result = xg_data_fetcher.get_match_xg_prediction("Team A", "Team B", "unsupported")
    assert result["refresh_status"] == "ready"
    assert result["availability"] == "unavailable"
