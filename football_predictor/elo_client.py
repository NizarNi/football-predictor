...
def fetch_team_elo_ratings():
    ...
    if _elo_cache["data"] and _elo_cache["timestamp"]:
        ...
        if cache_age < timedelta(hours=ELO_CACHE_DURATION_HOURS):
            ...
            logger.info(
                ...
                extra={"cache_age_minutes": cache_age_minutes},
            )
    ...
    try:
        ...
        if team_elo_ratings:
            ...
            logger.info(
                "Successfully fetched Elo ratings",
                extra={"team_count": len(team_elo_ratings)},
            )
            return team_elo_ratings
        else:
            logger.warning("No Elo ratings found in ClubElo data")
            return None
        ...
        if _elo_cache["data"]:
            logger.warning("Using expired Elo cache due to fetch error")
            return _elo_cache["data"]
        return None
...
def get_team_elo(team_name):
    ...
    if mapped_name and mapped_name in elo_ratings:
        logger.info(
            "Mapped team name to ClubElo alias",
            extra={
                "requested_name": team_name,
                "resolved_name": mapped_name,
                "elo": elo_ratings[mapped_name],
            },
        )
    ...
    if team_name in elo_ratings:
        logger.info(
            "Exact team name match found",
            extra={"team_name": team_name, "elo": elo_ratings[team_name]},
        )
    ...
    logger.info(
        "Elo rating unavailable for team",
        extra={"requested_name": team_name},
    )
    return None
...
if __name__ == "__main__":
    ...
    if ratings:
        ...
        logger.info(
            "Sample Elo ratings",
            extra={"samples": {team: elo for team, elo in sample}},
        )

    # Test specific team lookup
    arsenal_elo = get_team_elo("Arsenal")
    if arsenal_elo:
        logger.info("Arsenal Elo rating", extra={"elo": arsenal_elo})
    ...
    if arsenal_elo:
        ...
        if city_elo:
            ...
            if probs:
                logger.info(
                    "Probability calculation sample",
                    extra={
                        "home_team": "Arsenal",
